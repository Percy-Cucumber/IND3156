...
c
      answer = monte(f, xmin, xmax, ymin, ymax, total)
...
c
      double precision function monte(func, x1, x2, y1, y2, nPoints)
      integer nPoints
      double precision func, x1, x2, y1, y2
      ...
      if ( (nin + nout).ne. nPoints ) then
       ...
       monte = (x2-x1)*(y2-y1)*dble(cntr)/dble(nPoints)
      end if
      return
      end function monte
c
